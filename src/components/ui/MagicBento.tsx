...
const MOBILE_BREAKPOINT = 480; // Reduced from 768px to allow animations on more devices
  ...
  const animateParticles = useCallback(() => {
    ...
    memoizedParticles.current.forEach((particle, index) => {
      const timeoutId = setTimeout(() => {
        ...
        // Set initial color explicitly to avoid "rgb(0, 0, 0)" to "currentColor" warning
        const cloneElement = clone as HTMLElement;
        cloneElement.style.color = `rgba(${glowColor}, 1)`;
        ...
        gsap.to(clone, {
          ...
          repeat: -1,
          yoyo: true,
          color: `rgba(${glowColor}, 0.3)`, // Explicit target color
        });
      }, index * 100);
...
const MagicBento: React.FC<MagicBentoProps> = ({
  ...
  // Only disable animations if explicitly requested, not based on mobile detection
    ...
    <>
      ...
      <BentoCardGrid gridRef={gridRef}>
        {cardData.map((card, index) => {
          // Generate a deterministic ID for each card based on its content
          const uniqueId = `card-${index}-${card.title.replace(/\s+/g, '-').toLowerCase()}`;
            ...
            <div
              ...
              ref={(el) => {
                ...
                const handleClick = (e: MouseEvent) => {
                  ...
                  // Set initial color explicitly to avoid "rgb(0, 0, 0)" to "currentColor" warning
                  ripple.style.color = `rgba(${glowColor}, 1)`;
                  ripple.style.backgroundColor = `rgba(${glowColor}, 0.1)`;
